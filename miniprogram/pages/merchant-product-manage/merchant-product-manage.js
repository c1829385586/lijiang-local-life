// pages/merchant-product-manage/merchant-product-manage.js
Page({
  ...
  async loadProducts() {
    ...
    try {
      const { result } = await wx.cloud.callFunction({ name: 'merchant', data: { action: 'listProducts' } })
      if (result.code === 0) this.setData({ products: result.data })
  ...
  async toggleStatus(e) {
    ...
    try {
      await wx.cloud.callFunction({ name: 'merchant', data: { action: 'updateProduct', productId: id, productData: { status: newStatus } } })
  ...
  async deleteProduct(e) {
    ...
    try {
      await wx.cloud.callFunction({ name: 'merchant', data: { action: 'deleteProduct', productId: id } })
